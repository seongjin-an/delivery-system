...
- [x] **DE-04 수락은 이걸로도 다 안 막힌다.** 레디스에 ACCEPTED 를 쓴 다음 발행이 실패하면 라이더는 500 을 받고,
      ...
      **→ 2단계에서 실제로 났다.** 주문 초당 20건에 dispatch-engine 을 kill -9 로 세 번 죽였더니 좀비가 2, 1, 0건 남았다.
      전부 보드는 ACCEPTED 인데 수락 시각이 죽인 바로 그 초였고 dispatch.assigned 가 안 나갔다. MySQL 로 옮겨서 수락과 아웃박스를
      한 트랜잭션에 묶으니 0, 0, 0건이었다(`.reference/tech-choice.md` 5절). 5단계까지 미루지 말고 다음에 레디스 아웃박스로 고친다.
      **→ 레디스 아웃박스로 고쳤다.** 수락 Lua 가 ACCEPTED 를 쓰면서 배차 확정 이벤트 두 개를 `dispatch:outbox` 에 같이 넣고,
      dispatch-engine 의 `DispatchOutboxRelay` 가 50ms 마다 꺼내 보낸다(보내다 죽으면 inflight 에 남았다가 30초 뒤 다시 나간다).
      *확인: 주문 초당 20건에 dispatch-engine 을 kill -9 로 여섯 번 죽였더니 좀비 0건(고치기 전 세 번에 2, 1, 0건).
      죽인 바로 그 초(16:35:15.225)에 수락된 주문이 ASSIGNED 로 넘어갔고, 새로 뜬 인스턴스가 죽은 쪽이 보내다 만 이벤트 1건을 다시 보냈다.*
      *처음엔 릴레이가 한 건씩 보내고 기다렸다. 카프카 응답이 수십~수백 ms 라 밀려서 배차 확정이 최대 6.4초 늦었고, 5초 뒤 픽업하러 온
      라이더 45명이 INVALID_STATE 를 받았다. 먼저 다 보내놓고 한꺼번에 기다리게 바꾸니 p99 2.2초, INVALID_STATE 0건.*
- [ ] **아웃박스 때문에 배차 확정이 늦게 나간다.** 예전엔 수락 요청이 그 자리에서 보내서 몇 ms 였는데, 지금은 릴레이를 거쳐
      p50 102ms, p99 2.2초다(주문 초당 20건). 수락 요청이 바로 한 번 보내보고, 릴레이는 몇 초 넘게 안 나간 것만 챙기는 식으로
      바꾸면 평소엔 예전만큼 빠르다. 같은 이벤트가 두 번 가는 일이 늘지만 order-api 가 걸러준다.
- [ ] **배차 실패 이벤트는 아직 바로 보낸다.** dispatch-engine 의 `NO_CANDIDATE`, `ALL_CANDIDATES_TAKEN` 과 offer-relay 의 `MAX_ATTEMPTS`,
      `NO_CANDIDATE_LEFT` 는 보드에 FAILED 를 쓴 뒤 카프카로 바로 보낸다. 그 사이에 죽으면 수락 때와 똑같이 주문이 DISPATCHING 으로 남는다.
      같은 아웃박스로 옮기려면 FAILED 를 쓰는 것도 Lua 로 바꿔야 한다(지금은 HSET 한 줄이다).
...
- [x] 완료 조건 확인 — 시뮬레이터로 라이더 100명 3초 주기에 초당 32.5~33.6 건 (rider-simulator 절 D1)
...
- [x] **재시작하면 밀린 좌표를 전부 다시 쓴다.** 위의 `latest` 는 커밋된 오프셋이 아예 없을 때만 먹는다.
      2단계 실험에서 geo-indexer 를 5분 끄고 다시 켜니 커밋한 자리부터 90만 건을 다시 읽으면서
      **310초 묵은 좌표**를 레디스에 썼다. 따라잡는 53초 동안 라이더들이 5분 전 자리부터 빨리감기로 다시 지나갔다.
      래빗엠큐 TTL 5초를 걸었을 때는 가장 묵은 게 5.2초였다(`.reference/tech-choice.md` 2절).
      → 레코드 타임스탬프가 10초(`max-age`) 넘은 좌표는 JSON 을 읽기 전에 버린다. sentAt(폰 시계)이 아니라 레코드
      타임스탬프(location-ingest 서버 시계)를 보는 건, 시계가 늦은 폰의 좌표가 통째로 버려지지 않게 하려고다.
      *확인: 초당 3000건에 geo-indexer 를 2분 끄고 켰더니 35만 7천 건을 버렸고, 다시 뜬 지 5초 만에 라이더 위도가
      끄기 직전 자리(37.45702)에서 2분 뒤 자리(37.47988)로 한 번에 뛰었다. 중간 자리를 다시 지나가지 않았다.
      시나리오 B 처럼 랙이 쌓이는 걸 그대로 봐야 할 땐 `LOCATION_MAX_AGE=0` 으로 끈다.*
...
- [x] `RE-01` 래빗엠큐 토폴로지 선언 — `libs/common` 으로 옮겼다 (아래 참고)
      *왜: 이게 래빗엠큐를 쓰는 이유 전부다. "특정 한 명에게, 10초 안에, 안 받으면 다음 사람" 을
      카프카로는 못 만든다.*
      *→ 2단계에서 만들어보니 만들 수는 있었다. "전부"는 아니고 이유가 바뀌었다(`.reference/tech-choice.md` 3절).*
...
- [ ] **재시작 직후 30초 동안 재제안이 느리다.** 2단계 실험에서 offer-relay 를 막 띄운 채로 주문 초당 10건을 넣었더니,
      JVM 이 식어 있는 첫 30초 동안 재제안을 초당 1~9건밖에 못 해서(데워진 뒤엔 40~78건) 10초 타이머의 p95 가 24초가 됐다.
      점심 피크에 배포하면 그대로 나온다. 5단계에서 배포 중 부하를 걸어볼 때 같이 본다(워밍업 요청이나 롤링 순서).
...
- [ ] **제안 푸시를 마케팅과 다른 큐로 뺀다.** 2단계 실험에서 마케팅 1만 건 뒤에 제안을 넣어보니, 카프카로 제안 전용 토픽과
      컨슈머를 따로 뒀을 때 제안 푸시가 0.15~0.29초 만에 나갔다. 래빗엠큐 우선순위 큐(prefetch 20)는 2.6~2.8초,
      main 기본값(prefetch 250)은 한 번도 못 나갔다(`.reference/tech-choice.md` 3절). 우선순위는 큐 안에서만 앞지르고
      컨슈머가 이미 쥔 건 못 앞지른다. 래빗엠큐에서도 `notify.push.offer` 큐를 따로 두고 재본다. 시나리오 C-1 과 같이 볼 것.
...
- [ ] **수락이 서버에선 됐는데 시뮬레이터가 실패로 보면 주문이 ASSIGNED 로 영영 남는다.** 2단계 배차 상태 실험의 워밍업 판에서
      막 뜬 서비스가 느려 시뮬레이터가 3초 타임아웃으로 수락 49건을 실패 처리했는데, 서버에선 수락이 돼 있었다. 시뮬레이터는
      픽업을 안 가고, 주문은 ASSIGNED, 라이더는 DELIVERING 으로 남았다(한 판에 주문 62건, 라이더 87명). 실제 앱에서도 응답이
      유실되면 똑같다. 시뮬레이터는 실패한 수락과 픽업을 조회로 확인하고 다시 시도해야 하고, 서버 쪽엔 ASSIGNED 인데 오래 픽업이
      없는 주문을 찾는 장치가 필요하다.
...
- [x] **FAILED 주문은 DB 의 attempt 가 0 이다.** OR-07 이 FAILED 를 반영할 때 attempt 를 안 채우고, `dispatch.failed` 에도 attempt 가 없다.
      다섯 번 제안했는데 조회하면 0 으로 보인다. → `DispatchFailed` 에 attempt 를 실어서 고쳤다(order-api 절 OR-07, PR #17).
...
- [x] **위치 스트림을 래빗엠큐로 바꿔본다** — 결과는 `.reference/tech-choice.md` 2절, 코드는 `exp/location-rabbitmq` 브랜치
      *예상: 큐가 메모리를 먹다가 flow control 이 걸리고, 컨슈머가 밀리면 브로커가 먼저 죽는다.*
      *카프카는 디스크에 순차 append 하고 컨슈머는 오프셋만 들고 있어서 밀려도 브로커는 멀쩡하다.*
      *실제: 반만 맞았다. 래빗엠큐 4 는 90만 건이 밀려도 메모리가 68MB 만 늘었다(클래식 큐가 바로 디스크로 내린다).
      대신 디스크를 카프카의 9배 쓰고 상한이 없어서, 결국 디스크 알람이 걸리면 같은 브로커의 배차 제안까지 막힌다.
      알람을 억지로 걸어보니 브로커가 아니라 location-ingest 가 먼저 멈췄다 — 톰캣 스레드 200개가 연결 하나의 쓰기 락에
      다 묶였고, 실패 지표는 0 이었고, 풀린 뒤 순서가 1만 5천 건 뒤집혔다. 래빗엠큐가 나은 것도 하나 있었다.
      TTL 5초를 걸면 밀렸다 따라잡을 때 묵은 좌표를 브로커가 알아서 버려준다.*
- [x] **배차 제안을 카프카로 바꿔본다** — 결과는 `.reference/tech-choice.md` 3절, 코드는 `exp/offer-kafka` 브랜치
      *예상: "이 라이더에게만, 10초 TTL" 을 표현할 방법이 없다. 스케줄러 테이블을 따로 만들게 되는데,*
      *그게 곧 래빗엠큐의 TTL+DLX 를 손으로 재구현하는 것이라는 걸 알게 된다.*
      *실제: 틀렸다. 타이머 컨슈머가 레코드마다 offeredAt+10초까지 자다가 만료 토픽으로 넘기면 됐다(44줄). TTL 이 하나라
      파티션 안에서 먼저 온 게 먼저 만료되기 때문이다. p99 는 카프카 10.9초, 래빗엠큐 10.1초 안. kill -9 로 죽였다 살려도
      둘 다 주문을 잃지 않았다. 차이는 카프카가 죽은 컨슈머를 알아채는 데 45초(session.timeout.ms)가 걸린다는 것.
      "이 사람에게만" 은 원래 HTTP 로 쏘는 워커 몫이라 브로커와 상관없었다. 래빗엠큐는 그대로 두되, 이유는 "못 만든다" 가
      아니라 "타이머가 설정이고, TTL 이 여러 개여도 큐만 늘리면 되고, 죽었을 때 바로 되돌린다" 로 바뀌었다.
      덤으로, 제안 전용 줄을 따로 세우는 게 우선순위 큐보다 빨랐다.*
- [x] **레디스 GEO 없이 MySQL 공간 인덱스로 후보를 찾아본다** — 결과는 `.reference/tech-choice.md` 4절, 코드는 `exp/geo-mysql` 브랜치
      *예상: 주문 하나당 쿼리 하나. 초당 200 주문이면 DB 가 먼저 눕는다.*
      *실제: 맞았는데 더 일찍 누웠다. 좌표 쓰기가 없으면 검색이 초당 170건에서, 라이더 1만 명의 좌표 쓰기(초당 3333건)가 붙으면
      48건에서 멈췄다. 레디스는 같은 조건에서 초당 200건을 p99 267ms 로 받았다. 그보다 먼저 쓰기가 막혔다 — MySQL upsert 가
      CPU 한 코어를 채우고 geo-indexer 랙이 28,000건에서 안 줄어서 묵은 좌표를 버리기 시작했다.
      덤으로 두 개를 찾았다. dispatch-engine 이 검색마다 레디스 연결을 새로 열어 초당 1000건에서 포트가 바닥났다(PR #23 에서 고침).
      그리고 공간 컬럼 DDL(`SRID 4326`)을 Debezium 이 못 읽어서 주문 이벤트가 통째로 멈췄다.*
- [x] **배차 상태를 MySQL 로 옮겨본다** — 결과는 `.reference/tech-choice.md` 5절, 코드는 `exp/dispatch-state-mysql` 브랜치
      ...
      *실제: Lua 네 개(주석 포함 151줄)가 SQL 문장 여덟 개가 됐고 판정도 같았다(수락과 만료를 동시에 100번 날려도 한쪽만 이겼다).
      대신 느렸다. 주문 초당 20건에서 수락 p50 33 → 130ms, 배차 p99 437 → 912ms, 그리고 배차가 밀려서 배달 완료가 1070 → 545건.
      제일 값진 건 트랜잭션이었다. dispatch-engine 을 kill -9 로 세 번 죽였더니 레디스 판은 좀비가 2, 1, 0건(전부 ACCEPTED 인데
      dispatch.assigned 가 안 나간 주문), 수락과 아웃박스를 한 트랜잭션에 묶은 MySQL 판은 0, 0, 0건이었다. 배차 상태는 레디스에 두고,
      레디스 아웃박스를 만든다(1단계 DE-04 항목).*
- [x] 결과를 `.reference/tech-choice.md` 에 표로 정리 — "왜 이 셋을 같이 쓰는지" 에 대한 답 (6절)
