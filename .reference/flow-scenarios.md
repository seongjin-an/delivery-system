...
**왜 카프카로는 이걸 못 만드나.** 카프카에는 "이 메시지를 10초 뒤에 꺼내라"가 없다.
메시지는 순서대로 쌓이고 컨슈머는 처음부터 순서대로 읽는다. 그래서 지연 처리를 하려면
`(주문, 만료시각)` 을 담은 스케줄 테이블을 따로 만들고, 그걸 1초마다 훑는 폴러를 붙여야 한다.
만들 수는 있지만 그게 정확히 래빗엠큐의 TTL과 DLX를 손으로 다시 만드는 일이다.
반대로 위치 스트림을 래빗엠큐에 넣으면 큐가 메모리를 먹다가 브로커가 먼저 눕는다.
두 브로커를 같이 쓰는 이유가 여기서 갈린다.
